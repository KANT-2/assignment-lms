...
> **assignment-lms 적용**: 위 `templates/` 는 이 프로젝트에서 `apps/common/templates/` 에 해당한다.
> `apps/common/templates/base.html`, `apps/common/templates/includes/{sidebar,topbar,messages}.html`.
> 학생/튜터 앱 템플릿은 `{% extends "base.html" %}` 만 하고 shell 을 다시 만들지 않는다.
> 사이드바 네비게이션은 로그인 사용자 role(STUDENT/TUTOR)에 따라 항목이 갈리며, 정의는 `sidebar.html` 한 곳에서만 관리한다.
