...
| `tutor-dashboard.html` | 튜터 대시보드 — 좌측 사이드바 내비, 한 줄 요약, 진행 중인 과제 현황, 피드백 대기 목록, 미제출 학생 현황 + Slack 독려 모달, 강의안 준비 현황(차시별 영상·교안 등록 상태 — 교안은 선택). 목록형 박스는 항목이 많아지면 카드 안에서 스크롤 | PRD 7장 | 튜터팀 공용 |
| `tutor-student-management.html` | 튜터 학생 관리 — (A) 전체 학생 명단 + 필수·선택 제출률·마지막 제출, (B) 학생 클릭 → 과제별 제출 이력 상세. 모니터링 목적. 팀/성적은 자리만 (§5 참고) | PRD 7장 | 튜터팀 |
...
| `student-lecture.html` | 학생 — 강의 목록 및 시청(플레이어/준비중 분기) → 교안 자료 다운로드 / 블로그 요약 링크 | FR-016 | 강의 담당 |
| `tutor-lecture.html` | 튜터 — 차시(수업) 관리 목록 + 등록/수정 모달(교안 멀티 등록) + 유튜브 영상 링크 빠른 등록 + 삭제(undo) | FR-015 | 강의 담당 |
...
### 4. `tutor-dashboard.html` — Slack 독려 알림은 아직 인프라 없음

- **Slack 독려 알림**: Slack 연동(Incoming Webhook / Bot Token)·핸들 매핑(`accounts_user` ↔ Slack user)·발송 큐가 전부 미구현. 목업의 "연동됨" 뱃지와 발송 토스트는 UI 흐름 참고용. 실제로는 설정 화면 + 연동 저장소 + 발송 서비스가 필요.
- **사이드바**: 공통 shell(`apps/common/templates/includes/sidebar.html`)에 이미 있음 — 목업의 다크 사이드바는 배치/그룹핑 참고용이고 실제 색·마크업은 shell을 따른다.
- **강의안 준비 현황**: 학원 특성상 교안(`LessonMaterial`) 첨부는 튜터 재량 — "교안 없음"을 오류로 표시하지 말 것. `영상·교안 모두 없음` + 지난 차시일 때만 주의(amber)로 표시.

### 5. `tutor-student-management.html` — 자리만 잡은 컬럼 3개

목적: **모니터링**(누가 뒤처지나 · 미제출 관리). 단일 기수 운영이라 라운드 선택 UI 없음.

- **학생 명단**: `accounts.get_students()`(전체 승인 학생) 기준. 라운드별 팀 배정과 무관하게 전원 표시.
- **팀**: 라운드(프로젝트)마다 팀이 재편성되며 팀 명단은 **별도 테이블에서 관리 · 아직 미연동** → "미연동" pill. 연동되면 컬럼 채움.
- **성적**: 평가 집계 방식 미정 → 컬럼 자리만 (`—`).
- **제출률**: 지금은 **전체 과제 기준**(필수/선택 분리). 프로젝트별 제출률은 `Assignment`–프로젝트 연결(스키마 추가) 후.
- **마지막 제출**: 마지막 과제 제출(`Submission.submitted_at`) 시각. 접속·활동 시각은 어디에도 기록하지 않음 (체류시간 컬럼은 제거됨).
- **화면 B(학생 상세)**: 과제별 제출 이력. "제출물 보기"는 `tutor:submission-review` 또는 제출 상세로 연결.
